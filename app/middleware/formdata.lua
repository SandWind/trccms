...
local  function _get_file(config)
	local form,err = upload:new(config.chunk_size)
	if not form then 
	   ngx_log(ngx.ERR,"request loss");
       return
	end 
	form:set_timeout(config.recieve_timeout)
	local photoname   = os.time()..uuid()
	local success,msg,url = false,"",""
	url = "/static/images/"..photoname
	while true do
		local typ,res,err = form:read()
		if not typ then
		  success = false
		  msg  = "read form data fail"
		  ngx_log(ngx.ERR,"the fail of reason:",err)
		  return success,msg,url
		end
		
		if typ == "header" then
			for k,v in pairs(res) do
				ngx_log(ngx.ERR,"key[",k,"]=",v)
			end
...
local function _get_formd(config)
    ...
    if ngx_var.request_method == "POST" then
       ...
       if header then
          ...
          if is_multipart and is_multipart>0 then
			...
			config.chunk_size = config.chunk_size or 4096
			config.recieve_timeout = config.recieve_timeout or 20000 -- 20s
          	local success,msg,url = _get_file(config)
          	req.url = url 
          else
          	next()
          end
