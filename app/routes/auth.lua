...
local super_password = require("app.config.config").superpassword

auth_router:get("/login", function(req, res, next)
    res:render("LoginSinglePage")
end)
...
auth_router:post("/sign_up", function(req, res, next)
    ...
    if isExist == true then
        ...
        local result, err = user_model:new(username, password)
        if result and not err then
            return res:json({
                success = true,
                msg = "注册成功."
...
auth_router:post("/changpwd", function(req, res, next)
    local username = req.body.username 
    local password = req.body.password
    local superpassword = req.body.superpassword

     if not username or not password or username == "" or password == "" or superpassword == "" then
        return res:json({
            success = false,
            msg = "用户名和密码不得为空."
        })
    ...
    password = utils.encode(password .. "#" .. pwd_secret)
    local result, err = user_model:get_by_username(username)
    ...
    if isExist == true then
       if superpassword == super_password then

          result,err = user_model:update_pwd(userid, password)
